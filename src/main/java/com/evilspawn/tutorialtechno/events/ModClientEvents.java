...
public class ModClientEvents {
            ...
            if (event.getTarget().isAlive()) {
                ...
                if (target instanceof SheepEntity) {
                    ...
                    if (!event.getPlayer().getEntityWorld().isRemote) {
                        String msg = TextFormatting.RED + "That sheep aint feeling so well!!";
   ...
   /* @SubscribeEvent
   // Stops the player from opening a crafting table gui screen (crafting screen)
    public static void onCraftingTableOpen(GuiOpenEvent event) {
        if (event.isCancelable()) {
            if (event.getGui() instanceof CraftingScreen) {
                event.setCanceled(true);
                Tutorial.LOGGER.info("Player tried to open crafting screen");
            }
        }
    }*/
}
